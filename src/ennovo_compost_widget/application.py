...
from .app_state import EnnovoCompostWidgetState

log = logging.getLogger()

class EnnovoCompostWidgetApplication(Application):
    config: EnnovoCompostWidgetConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: EnnovoCompostWidgetUI = None
        self.state: EnnovoCompostWidgetState = None

    async def setup(self):
        self.ui = EnnovoCompostWidgetUI()
        self.state = EnnovoCompostWidgetState()
